//gcc pcmtoaac1.c -lfdk-aac -o pcmtoaac1
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <sys/time.h>
...
int main()
{
    ...
    /* 设置采样率 */
    if(aacEncoder_SetParam(aacEncHandle, AACENC_SAMPLERATE, 11025) != AACENC_OK )
    {
        return -1;
    }
    ...
    if (aacEncoder_SetParam(aacEncHandle, AACENC_BITRATE, 16000) != AACENC_OK)
    {
       return -1; 
    }
    ...
    FILE* rfd = fopen("audioNew.pcm", "rb");
    if(!rfd)
    {
        return -1;
    }

    FILE* wfd = fopen("audioNew.aac", "wb");
    if(!wfd)
    {
        return -1;
    }
